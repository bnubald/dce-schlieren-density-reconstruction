...
class analytical():
    ...
    def plot_sampling_pts_overlay(self, savefile=None):
        """
        Plot true density field with sampled points overlay using either gen_all_pts, 
            gen_subsample_pts, or gen_ran_pts
        ...
        fig, ax = plt.subplots(1, 1, figsize=(10, 10), gridspec_kw = {'wspace':0.2, 'hspace':0.05})

        value_a = density
        a = ax.imshow(value_a, interpolation=interp, cmap=gcmap,
                    origin='lower', extent=[Xo.min(), Xo.max(), Yo.min(), Yo.max()],
                    vmin=value_a.min(), vmax=value_a.max())
        ax.scatter(self.X[:,0], self.X[:,1], s=40, edgecolor='w', facecolors='None')
        ax.scatter(self.X_f[:,0], self.X_f[:,1], s=80, edgecolor='w', facecolors='green')

        # Hide ticks
        ax.axes.xaxis.set_visible(False)
        ax.axes.yaxis.set_visible(False)

        fig.colorbar(a, ax=ax, orientation='vertical', shrink=0.75)

        ax.set_title(r'True density ($\rho$)', fontdict={'fontsize': 26})
        # fig.tight_layout()
    ...
    def plot_posterior(self, savefile=None):
        ...
        # grid = pv.StructuredGrid( Xo, Yo, np.zeros_like(Xo) )
        # grid['rho_estimate'] = mu_s

        # # Calculate posterior density gradient
        # grad = grid.compute_derivative(scalars='rho_estimate')
        # mu_dx = grad['gradient'][:, 1].reshape(N, N)
        # mu_dy = grad['gradient'][:, 0].reshape(N, N)
        ...
        matplotlib.rcParams.update({'font.size': 30})
        fig, ((ax1, ax2, ax3), (ax4, ax5, ax6)) = plt.subplots(2, 3, figsize=(30, 20), gridspec_kw = {'wspace':0.2, 'hspace':0.05})
        axes = [ax1, ax1, ax2, ax3, ax4, ax5, ax6]
        ...
        ax3.text(0.5, -0.1, 'c)', transform=ax3.transAxes)
        ax4.text(0.5, -0.1, 'd)', transform=ax4.transAxes)
        ax5.text(0.5, -0.1, 'e)', transform=ax5.transAxes)
        ax6.text(0.5, -0.1, 'f)', transform=ax6.transAxes)
